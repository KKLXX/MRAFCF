...
class TextConfig(PretrainedConfig): # the config class for the text encoder model
...
class PersonalityConfig(PretrainedConfig):
    pretrained_model_archive_map = {}
    config_name = 'personality_config.json'
    def __init__(self,
                 vocab_size_or_config_json_file=4096,
                 hidden_size=768,
                 num_hidden_layers=3,
...
class TfSelfAttention(nn.Module):
    ...
    def transpose_for_scores(self, x): # the function to transpose the input tensor. [b, seq_len,num_attention_heads, attention_head_size] to [b, num_attention_heads, seq_len, attention_head_size]
...
class TfAttention(nn.Module): # the transformer attention module
    def __init__(self, config):
